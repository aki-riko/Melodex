...
assert.equal(shouldUsePlaybackFade(true), true, '窗口有焦点时媒体键应保留淡入淡出');
assert.equal(shouldUsePlaybackFade(false), false, '窗口失焦时媒体键应切换到后台安全控制');

const backgroundPauseAudio = {
  volume: 0.2,
  pauseCalls: 0,
  pause() { this.pauseCalls += 1; },
};
assert.equal(pauseAudioImmediately(backgroundPauseAudio, 0.75), true, '后台媒体键暂停应同步执行');
assert.equal(backgroundPauseAudio.pauseCalls, 1, '后台媒体键暂停不应等待动画帧');
assert.equal(backgroundPauseAudio.volume, 0.75, '立即暂停后应恢复用户音量供下次播放');

const backgroundResumeAudio = {
  volume: 0,
  playCalls: 0,
  volumeWhenPlayed: null,
  async play() {
    this.playCalls += 1;
    this.volumeWhenPlayed = this.volume;
  },
};
assert.equal(await resumeAudioImmediately(backgroundResumeAudio, 0.65), true, '后台媒体键播放应直接恢复');
assert.equal(backgroundResumeAudio.playCalls, 1, '后台媒体键播放不应等待动画帧');
assert.equal(backgroundResumeAudio.volumeWhenPlayed, 0.65, '调用 play 前必须先恢复用户音量');
