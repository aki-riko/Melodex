...
// 已下载页:下载到服务器的音乐 + 上传文件,按 user_id 归属过滤。
        ...
        <div className="space-y-0.5">
          {songs.map((song, i) => (
            <SongRow key={`${song.source}-${song.id}`} song={song} index={i}
              ...
              onRemove={handleDelete} removeTitle="从已下载列表删除" removeHint="删除服务器上的这首歌" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div>
      <div className="flex items-end gap-4 mb-6">
        ...
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">服务器音乐</p>
          <h1 className="text-3xl font-black truncate">已下载到服务器</h1>
      ...
      {data?.refreshing && (
        <div className="mb-4 inline-flex items-center gap-2 rounded-md border border-border bg-card/70 px-3 py-2 text-sm text-muted-foreground">
          ...
          <span>正在后台扫描已下载文件，当前先显示已有结果</span>
        </div>
      )}
      {!primaryLoading && tracks.length === 0 && (
        <p className="text-muted-foreground">还没有已下载歌曲。在搜索页下载歌曲、或在此上传文件后会出现在这里。</p>
      )}
