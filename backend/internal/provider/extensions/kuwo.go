...
func (client *Kuwo) SearchPlaylist(keyword string) ([]model.RemoteCollection, error) {
...
func (client *Kuwo) SearchAlbum(keyword string) ([]model.RemoteCollection, error) {
...
func (client *Kuwo) GetPlaylistSongs(id string) ([]model.Track, error) {
	_, songs, err := client.Playlist(id)
	return songs, err
}

func (client *Kuwo) GetAlbumSongs(id string) ([]model.Track, error) {
	_, songs, err := client.Album(id)
	return songs, err
}

func (client *Kuwo) Playlist(id string) (*model.RemoteCollection, []model.Track, error) {
...
func (client *Kuwo) Album(id string) (*model.RemoteCollection, []model.Track, error) {
	id = strings.TrimSpace(id)
	if id == "" {
...
func (client *Kuwo) ParsePlaylist(link string) (*model.RemoteCollection, []model.Track, error) {
...
func (client *Kuwo) ParseAlbum(link string) (*model.RemoteCollection, []model.Track, error) {
...
func (client *Kuwo) RecommendedPlaylists() ([]model.RemoteCollection, error) {
	query := url.Values{
...
func (client *Kuwo) PlaylistCategories() ([]model.RemoteCategory, error) {
	...
	result := []model.RemoteCategory{{ID: "", Name: "全部", Group: "全部", Source: "kuwo"}}
...
func (client *Kuwo) CategoryPlaylists(categoryID string, page, limit int) ([]model.RemoteCollection, error) {
	if page < 1 {
		page = 1
	}
...
func kuwoSongs(value interface{}, collection model.RemoteCollection) []model.Track {
	items := array(value)
...
func kuwoPlaylists(value interface{}) []model.RemoteCollection {
	items := array(value)
...
func kuwoPlaylistDetail(raw map[string]interface{}, fallbackID string) model.RemoteCollection {
...
func kuwoAlbums(value interface{}) []model.RemoteCollection {
	items := array(value)
	result := make([]model.RemoteCollection, 0, len(items))
	for _, item := range items {
...
func kuwoAlbum(raw map[string]interface{}) model.RemoteCollection {
...
func kuwoBrowsePlaylists(value interface{}) []model.RemoteCollection {
	items := array(value)
	result := make([]model.RemoteCollection, 0, len(items))
	for _, item := range items {
		raw := object(item)
